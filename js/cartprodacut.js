...
function drawProductUI(allProduct = []) {
    if ((JSON.parse(localStorage.getItem("prouductcard"))).length===0) {
        noproduct.innerHTML = "no produts in cart"
    } else {
        let Products = JSON.parse(localStorage.getItem("prouductcard")) || allProduct;
...
function removetocart(id) {
    let produtcincart = localStorage.getItem('prouductcard');
    if (produtcincart) {
        let items = JSON.parse(produtcincart);
        let filteredItems = items.filter((item) => item.id !== id);
        localStorage.setItem("prouductcard", JSON.stringify(filteredItems));
